...
#   name = "${var.environment}-${local.service_name}-secrets"
...
# Required so ECS can pull the MongoDB secret at container startup
# Without this the task will fail with AccessDeniedException before the container even starts
resource "aws_iam_role_policy" "secrets_read" {
  name = "${var.environment}-${local.service_name}-secrets-read"
  role = aws_iam_role.task_execution_role.id
